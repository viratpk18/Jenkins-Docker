pipeline {
    ...
    stages {
        stage('Checkout Code') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'git_viratpk18', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_TOKEN')]) { // change the credential id 
                    git url: "https://$GIT_USER:$GIT_TOKEN@github.com/viratpk18/Jenkins-Docker.git", branch: 'main' //change the repo
                }
            }
        }
